...
class ${name} extends AvenxComponent {
  ...
  extractStylesAndVars(desContent, desBlocks, desPath = '') {
    ...
    if (globalMatch) {
      ...
      globalStartLine = desContent.substring(0, idx).split('\n').length;
  ...
  extractTemplate(content, desBlocks, name, filePath, state, computed, methods) {
    ...
    const desPath = filePath ? filePath.replace(isPage ? '.page.js' : '.component.js', isPage ? '.page.css' : '.component.css') : '';
    template = this.styleProcessor.process(template, desBlocks, name, desPath);
  ...
  validateTemplate(template, state, computed, methods, filePath, name) {
    ...
    const checkIdentifiers = (expr) => {
      ...
      for (const id of ids) {
        ...
        if (!declared.has(id)) {
          logger.warn(formatMessage(AvenxErrorCodes.COMPILER_UNDECLARED_REFERENCE, id, filename));
        }
      }
    };
